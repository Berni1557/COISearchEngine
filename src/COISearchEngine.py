...
class DocSearch:
    ...
    def __init__(self, settingsfilepath='settings.yml'):
        ...
        self.yamlh = YAMLH()                          #: yaml handler

        # Create settings
        self.props = defaultdict(lambda: None,
            maxdist = 2,
            prefixlength = 1,
            folderpath_pdf='../documents',
            folderpath_result='../results',
        ...
        if os.path.isfile(settingsfilepath):
            print('Reading settings file:', settingsfilepath)
            self.props = self.yamlh.load(settingsfilepath)
        else:
            print('Creating settings file:', settingsfilepath)
            self.yamlh.save(self.props, self.settingsfilepath)
    ...
    def extractPDFText(self, filename_merge=''):
        ...
        with open(filepath_merge, 'rb') as file:
            reader = PdfFileReader(file)
            numpages = reader.getNumPages()
    ...
    def markResults(self, filename_merge='', filename_mark=''):
        ...
        print('Highlight results in pdf file in:', filepath_mark)
    ...
    def exportResults(self, filename_result=''):
        ...
        print('Export results to excel file to: ', filepath_result)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--filepath_seetings", help="Filepath to the seetings.yml file.", type=str, default='../settings/settings.yml')
    ...
    print('------------------------- Finished COISearchEngine -------------------------')
